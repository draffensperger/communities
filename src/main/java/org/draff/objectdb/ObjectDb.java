...
public interface ObjectDb {
  ...
  <T extends Model> void createOrUpdateById(Class<T> clazz, long id, ObjectUpdater<T> updater);
  <T extends Model> void createOrUpdateByIds(Class<T> clazz, List<Long> ids, ObjectUpdater<T> updater);
  <T extends Model> void createOrUpdateByNames(Class<T> clazz, List<String> names, ObjectUpdater<T> updater);
